...
type InterestsViewNavigationProp = NativeStackNavigationProp<ActivityStackParamList>;

type TabType = 'received' | 'sent' | 'accepted';

const InterestsView: React.FC = () => {
    const navigation = useNavigation<InterestsViewNavigationProp>();
    ...
    const loadMatches = async (isRefresh = false) => {
        ...
        try {
            console.log('Loading matches for tab:', activeTab);
            setError(null);
            let response;
            ...
            if (activeTab === 'received') {
                ...
                console.log('Accepted matches response:', JSON.stringify(response, null, 2));
            }

            // Add defensive check for matches array
            const matchesArray = response?.matches || [];
            console.log(`Loaded ${matchesArray.length} matches for ${activeTab}`);
    ...
    const handleAccept = async (requestId: number) => {
        ...
        try {
            ...
            // Wait for success animation then switch to accepted tab
            setTimeout(() => {
    ...
    const handleViewProfile = (userId: number) => {
        // @ts-ignore - Navigation typing might be tricky with nested stacks
    ...
    const renderAcceptedMatch = ({ item }: { item: MatchRequest }) => {
        ...
        console.log('Rendering accepted match:', {
            matchId: item.id,
            ...
            otherUserName: otherUser?.profile?.firstName
        });
    ...
    const renderEmpty = () => {
        ...
        const emptyMessages = {
            ...
            sent: {
                ...
                message:
                    'Start sending match requests to connect with profiles you like. Browse the Matches screen to find matches!',
